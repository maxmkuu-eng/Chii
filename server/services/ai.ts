...
export const AI_CONFIG = {
  ...
  imageGenModel: process.env.IMAGE_MODEL || "magic-hour-default",
  imageProvider: "magic_hour",
  searchProvider: "exa",
  searchEnabled: true,
...
export async function generateContentWithFallback(params: {
  ...
  const modelsToTry = [primaryModel, ...AI_CONFIG.fallbackTextModels.filter(m => m !== primaryModel)];
  ...
  for (let i = 0; i < modelsToTry.length; i++) {
    ...
    try {
      ...
      if (enableGrounding) {
        requestConfig.tools = [{ googleSearch: {} }, ...(requestConfig.tools || [])];
      }
      const response = await withTimeout(
        ai.models.generateContent({ model, contents: params.contents, config: requestConfig }),
        14000,
        `Model ${model} timed out after 14s`
      );
      const extractedText = extractTextFromGeminiResponse(response);
      if (extractedText && extractedText.trim()) {
        return { response, modelUsed: model, text: extractedText.trim(), sources: extractGroundingSources(response) };
      }
    } catch (err: any) {
      ...
      if (enableGrounding) {
        try {
          ...
          if (fbText && fbText.trim()) return { response: fallbackResponse, modelUsed: model, text: fbText.trim() };
        } catch {}
      }
      if (isQuotaError && i < modelsToTry.length - 1) await new Promise(resolve => setTimeout(resolve, 300));
    }
  }
...
export async function* streamContentWithFallback(params: {
  ...
  for (const model of modelsToTry) {
    try {
      const requestConfig = { ...(params.config || {}) };
      if (enableGrounding) requestConfig.tools = [{ googleSearch: {} }, ...(requestConfig.tools || [])];
      const responseStream = await ai.models.generateContentStream({ model, contents: params.contents, config: requestConfig });
      let emittedStart = false;
      let totalText = "";
      for await (const chunk of responseStream) {
        if (!emittedStart) {
          yield { text: "", model, isStart: true };
          emittedStart = true;
        }
        const chunkText = chunk.text || "";
        if (chunkText) {
          totalText += chunkText;
          yield { text: chunkText, model };
        }
      }
      if (totalText.trim()) {
